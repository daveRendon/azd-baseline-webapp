...
param logAnalytics object = {
  name: 'la-${uniqueString(subscription().subscriptionId, resourceGroup().id)}-${prefix}'
  skuName: 'PerGB2018'
}

@description('Deployment settings for Azure SQL and Azure SQL database instances.')
param azureSqlDatabase object = {
  serverName: 'sql-${uniqueString(subscription().subscriptionId, resourceGroup().id)}-${prefix}'
  databaseName: 'appdb'
...
param keyVault object = {
  name: 'kv-${uniqueString(subscription().subscriptionId, resourceGroup().id)}-${prefix}'
  skuName: 'standard'
  skuFamily: 'A'
}
...
param azureAppService object = {
  name: 'app-${uniqueString(subscription().subscriptionId, resourceGroup().id)}-${prefix}'
  webSiteName: 'app-${uniqueString(subscription().subscriptionId, resourceGroup().id)}-${prefix}'
  skuName: 'S1'
  skuCapacity: 1
  autoScaleCpuMax: '80'
  autoScaleCpuMin: '60'
...
resource azureSqlDatabase_serverName_azureSqlDatabase_database 'Microsoft.Sql/servers/databases@2020-08-01-preview' = {
  parent: azureSqlDatabase_server
  name: '${azureSqlDatabase.databaseName}'
  location: location
}
